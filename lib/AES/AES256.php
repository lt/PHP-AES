...
class AES256 extends AESBase
{
    function setKey($key)
    {
        ...
        $this->rk = [
            ...
            $rk4 = $rk4 ^ ($s[$rk3 & 0xff] | ($s[$rk3 >> 8 & 0xff] << 8) | ($s[$rk3 >> 16 & 0xff] << 16) | ($s[$rk3 >> 24 & 0xff] << 24)),
            $rk5 = $rk5 ^ $rk4, $rk6 = $rk6 ^ $rk5, $rk7 = $rk7 ^ $rk6,
            $rk0 = $rk0 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x02) << 24)),
            $rk1 = $rk1 ^ $rk0, $rk2 = $rk2 ^ $rk1, $rk3 = $rk3 ^ $rk2,
            $rk4 = $rk4 ^ ($s[$rk3 & 0xff] | ($s[$rk3 >> 8 & 0xff] << 8) | ($s[$rk3 >> 16 & 0xff] << 16) | ($s[$rk3 >> 24 & 0xff] << 24)),
            $rk5 = $rk5 ^ $rk4, $rk6 = $rk6 ^ $rk5, $rk7 = $rk7 ^ $rk6,
            $rk0 = $rk0 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x04) << 24)),
            $rk1 = $rk1 ^ $rk0, $rk2 = $rk2 ^ $rk1, $rk3 = $rk3 ^ $rk2,
            $rk4 = $rk4 ^ ($s[$rk3 & 0xff] | ($s[$rk3 >> 8 & 0xff] << 8) | ($s[$rk3 >> 16 & 0xff] << 16) | ($s[$rk3 >> 24 & 0xff] << 24)),
            $rk5 = $rk5 ^ $rk4, $rk6 = $rk6 ^ $rk5, $rk7 = $rk7 ^ $rk6,
            $rk0 = $rk0 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x08) << 24)),
            $rk1 = $rk1 ^ $rk0, $rk2 = $rk2 ^ $rk1, $rk3 = $rk3 ^ $rk2,
            $rk4 = $rk4 ^ ($s[$rk3 & 0xff] | ($s[$rk3 >> 8 & 0xff] << 8) | ($s[$rk3 >> 16 & 0xff] << 16) | ($s[$rk3 >> 24 & 0xff] << 24)),
            $rk5 = $rk5 ^ $rk4, $rk6 = $rk6 ^ $rk5, $rk7 = $rk7 ^ $rk6,
            $rk0 = $rk0 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x10) << 24)),
            $rk1 = $rk1 ^ $rk0, $rk2 = $rk2 ^ $rk1, $rk3 = $rk3 ^ $rk2,
            $rk4 = $rk4 ^ ($s[$rk3 & 0xff] | ($s[$rk3 >> 8 & 0xff] << 8) | ($s[$rk3 >> 16 & 0xff] << 16) | ($s[$rk3 >> 24 & 0xff] << 24)),
            $rk5 = $rk5 ^ $rk4, $rk6 = $rk6 ^ $rk5, $rk7 = $rk7 ^ $rk6,
            $rk0 = $rk0 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x20) << 24)),
            $rk1 = $rk1 ^ $rk0, $rk2 = $rk2 ^ $rk1, $rk3 = $rk3 ^ $rk2,
            $rk4 = $rk4 ^ ($s[$rk3 & 0xff] | ($s[$rk3 >> 8 & 0xff] << 8) | ($s[$rk3 >> 16 & 0xff] << 16) | ($s[$rk3 >> 24 & 0xff] << 24)),
            $rk5 = $rk5 ^ $rk4, $rk6 = $rk6 ^ $rk5, $rk7 = $rk7 ^ $rk6,
            $rk0 = $rk0 ^ ($s[$rk7 >> 24 & 0xff] | ($s[$rk7 & 0xff] << 8) | ($s[$rk7 >> 8 & 0xff] << 16) | (($s[$rk7 >> 16 & 0xff] ^ 0x40) << 24)),
            $rk1 = $rk1 ^ $rk0, $rk2 = $rk2 ^ $rk1, $rk3 = $rk3 ^ $rk2
        ];
    ...
    function encrypt($block)
    {
        ...
        return pack('N4',
            ($x0 & 0xff000000) ^ ($x1 & 0xff0000) ^ ($x2 & 0xff00) ^ ($x3 & 0xff) ^ $rk[56],
            ($x1 & 0xff000000) ^ ($x2 & 0xff0000) ^ ($x3 & 0xff00) ^ ($x0 & 0xff) ^ $rk[57],
            ($x2 & 0xff000000) ^ ($x3 & 0xff0000) ^ ($x0 & 0xff00) ^ ($x1 & 0xff) ^ $rk[58],
            ($x3 & 0xff000000) ^ ($x0 & 0xff0000) ^ ($x1 & 0xff00) ^ ($x2 & 0xff) ^ $rk[59]
        );
    }
